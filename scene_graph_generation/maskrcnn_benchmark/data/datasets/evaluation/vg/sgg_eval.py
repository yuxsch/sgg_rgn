...
class SGRecall(SceneGraphEvaluation):
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        local_container['gt_triples'] = gt_triplets
        local_container['pred_triples'] = pred_triplets
        # print('testtt')
        # print(pred_triplets)
        # print(gt_triplets)
...
class SGZeroShotRecallWithDetail(SceneGraphEvaluation):
    def __init__(self, result_dict, ind_to_predicates, rel_num):
        super(SGZeroShotRecallWithDetail, self).__init__(result_dict)
        self.rel_name_list = ind_to_predicates[1:]  # remove __background__
        self.num_rel = rel_num

    def register_container(self, mode):
        self.result_dict[mode + '_zero_recall'] = {20: [], 50: [], 100: []}
        self.result_dict[mode + '_zero_recall_collect'] = {20: [[] for i in range(self.num_rel)],
                                                           50: [[] for i in range(self.num_rel)],
                                                           100: [[] for i in range(self.num_rel)]}
        self.result_dict[mode + '_zero_recall_list'] = {20: [], 50: [], 100: []}


    def collect_zero_recall_items(self, global_container, local_container, mode):
        pred_to_gt = local_container['pred_to_gt']
        gt_rels = local_container['gt_rels']
        print('testttt')
        print(pred_to_gt.shape)
        print(gt_rels.shape)

        for k in self.result_dict[mode + '_zero_recall_collect']:
            # the following code are copied from Neural-MOTIFS
            match = reduce(np.union1d, pred_to_gt[:k])
            ...
            for idx in range(gt_rels.shape[0]):
                if len(self.zeroshot_idx) > 0 and int(match_list[idx]) in self.zeroshot_idx:
            ...
            for n in range(self.num_rel):
                if recall_count[n] > 0:
                    self.result_dict[mode + '_zero_recall_collect'][k][n].append(float(recall_hit[n] / recall_count[n]))

    # def calculate_recall(self, global_container, local_container, mode):
    #     pred_to_gt = local_container['pred_to_gt']
    #     for k in self.result_dict[mode + '_zero_recall']:
    #         # Zero Shot Recall
    #         match = reduce(np.union1d, pred_to_gt[:k])
    #         if len(self.zeroshot_idx) > 0:
    #             if not isinstance(match, (list, tuple)):
    ...
    def calculate_zero_recall(self, mode):
        for k, v in self.result_dict[mode + '_zero_recall'].items():
            sum_recall = 0
            num_rel_no_bg = self.num_rel - 1
            for idx in range(num_rel_no_bg):
                if len(self.result_dict[mode + '_zero_recall_collect'][k][idx + 1]) == 0:
                    tmp_recall = 0.0
                else:
                    tmp_recall = np.mean(self.result_dict[mode + '_zero_recall_collect'][k][idx + 1])
                self.result_dict[mode + '_zero_recall_list'][k].append(tmp_recall)
                sum_recall += tmp_recall

            self.result_dict[mode + '_zero_recall'][k] = sum_recall / float(num_rel_no_bg)
        return
    ...
    def generate_print_string(self, mode):
        ...
        result_str += '\n'
        result_str += '----------------------- Details ------------------------\n'
        for n, r in zip(self.rel_name_list, self.result_dict[mode + '_zero_recall_list'][20]):
            result_str += '({}:{:.4f}) '.format(str(n), r)
        result_str += '\n'
        result_str += '--------------------------------------------------------\n'
        for n, r in zip(self.rel_name_list, self.result_dict[mode + '_zero_recall_list'][50]):
            result_str += '({}:{:.4f}) '.format(str(n), r)
        result_str += '\n'
        result_str += '--------------------------------------------------------\n'
        for n, r in zip(self.rel_name_list, self.result_dict[mode + '_zero_recall_list'][100]):
            result_str += '({}:{:.4f}) '.format(str(n), r)
        result_str += '\n'
        result_str += '--------------------------------------------------------\n'
        return result_str
