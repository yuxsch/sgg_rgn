...
from maskrcnn_benchmark.data.datasets.evaluation.vg.sgg_eval import SGRecall, SGNoGraphConstraintRecall, SGZeroShotRecall, SGNGZeroShotRecall, SGPairAccuracy, SGMeanRecall, SGNGMeanRecall, SGAccumulateRecall, ZSMeanRecall
...
def do_vg_evaluation(
    ...
    if len(cfg.DATASETS.ZERO_SHOT_CONFIG_PATH) > 0:
        zeroshot_triplet = torch.load(Path("maskrcnn_benchmark/data/datasets/evaluation/vg")/Path(cfg.DATASETS.ZERO_SHOT_CONFIG_PATH), map_location=torch.device("cpu"))
    ...
    use_single_predicate = False
    predicate_num = 1

    if use_single_predicate:
        predi = zeroshot_triplet[:, 2]
        filter_idxes = np.where(predi == predicate_num)
        zeroshot_triplet = zeroshot_triplet[filter_idxes]
    ...
    if "relations" in iou_types:
        ...
        eval_zs_mean_recall = ZSMeanRecall(result_dict, num_rel_category, dataset.ind_to_predicates, print_detail=True)
        eval_zs_mean_recall.register_container(mode)
        evaluator['eval_zs_mean_recall'] = eval_zs_mean_recall
        ...
        global_container['pred_triples'] = []
        global_container['gt_triples'] = []
        global_container['pred_to_gt'] = []
        ...
        eval_zs_mean_recall.calculate_mean_recall(mode)
        ...
        result_str += eval_zs_mean_recall.generate_print_string(mode)
    ...
    # print('testtt')
    # print(global_container['gt_triples'])
    # print(global_container['pred_triples'])
    # torch.save(global_container['gt_triples'], os.path.join('./', 'gt_triples.pytorch'))
    # torch.save(global_container['pred_triples'], os.path.join('./', 'pred_triples.pytorch'))

    if "relations" in iou_types:
        if output_folder:
            torch.save(result_dict, os.path.join(output_folder, 'result_dict.pytorch'))
            torch.save(global_container['gt_triples'], os.path.join(output_folder, 'gt_triples.pytorch'))
            torch.save(global_container['pred_triples'], os.path.join(output_folder, 'pred_triples.pytorch'))
            torch.save(global_container['pred_to_gt'], os.path.join(output_folder, 'pred_to_gt.pytorch'))
...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator, zero_shot=False):
    ...
    # local_container['pred_triples'] = []
    # local_container['gt_triples'] = []
    ...
    global_container['pred_triples'].append(local_container['pred_triples'])
    global_container['gt_triples'].append(local_container['gt_triples'])
    global_container['pred_to_gt'].append(local_container['pred_to_gt'])
    return 
