...
class KGCContext(nn.Module):
    def __init__(self,config):
        super(KGCContext, self).__init__()
        kg_emb_np = np.load(config.GLOVE_DIR + '/kg_emb.npy')
        self.kg_emb = torch.from_numpy(kg_emb_np)
        self.kg_emb = self.kg_emb.cuda()

    def forward(self, pred_pairs):
        fir = pred_pairs[:,0] - 1
        sec = pred_pairs[:,1] - 1
        ans = self.kg_emb[fir,sec]

        zeroco = torch.zeros(ans.shape[0])
        zeroco = zeroco.cuda()
        # ans = ans.cuda()
        zeroco = zeroco.unsqueeze(1)
        ans = torch.cat((zeroco,ans),1)

        # ans = torch.softmax(ans,1)
...
class TransEmbedding(nn.Module):
    def __init__(self, config):
        ...
        kg_emb_np = np.load(config.GLOVE_DIR + '/kg_emb.npy')
        kg_emb = torch.from_numpy(kg_emb_np)
        self.num_entity = kg_emb.shape[0]
        self.predicate_dim = kg_emb.shape[2]+1
        zeroco = torch.ones(self.num_entity,self.num_entity)*0.5
        zeroco = zeroco.unsqueeze(dim=2)
        kg_emb = torch.cat([zeroco,kg_emb],dim=2)
        kg_emb = kg_emb.view(self.num_entity*self.num_entity,self.predicate_dim)
        self.trans_embedding = nn.Embedding(self.num_entity*self.num_entity,self.predicate_dim)
        self.dropout = nn.Dropout(0.2)
        with torch.no_grad():
            self.trans_embedding.weight.copy_(kg_emb, non_blocking=True)

    def forward(self, pred_pairs):
        fir = pred_pairs[:, 0] - 1
        sec = pred_pairs[:, 1] - 1
        ans = self.trans_embedding(fir*self.num_entity + sec)
