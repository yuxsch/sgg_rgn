...
from .model_motifs import LSTMContext, FrequencyBias, KGCContext, TransEmbedding
...
@registry.ROI_RELATION_PREDICTOR.register("TransformerKGCPredictor")
class TransformerKGCPredictor(nn.Module):
    def __init__(self, config, in_channels):
        super(TransformerKGCPredictor, self).__init__()
        ...
        self.kg_layer = KGCContext(config)
        # post decoding
        self.hidden_dim = config.MODEL.ROI_RELATION_HEAD.CONTEXT_HIDDEN_DIM
        self.pooling_dim = config.MODEL.ROI_RELATION_HEAD.CONTEXT_POOLING_DIM
        self.post_emb = nn.Linear(self.hidden_dim, self.hidden_dim * 2)
        self.post_cat = nn.Linear(self.hidden_dim * 2 + self.num_rel_cls, self.pooling_dim)
        self.rel_compress = nn.Linear(self.pooling_dim, self.num_rel_cls)
        self.ctx_compress = nn.Linear(self.hidden_dim * 2 + self.num_rel_cls, self.num_rel_cls)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        kg_dists = self.kg_layer(pair_pred)
        if kg_dists.dtype != prod_rep.dtype:
            kg_dists = kg_dists.to(prod_rep.dtype)
        combined_feature = torch.cat((prod_rep, kg_dists), dim=1)
        ctx_gate = self.post_cat(combined_feature)
        ...
        rel_dists = self.rel_compress(visual_rep) + self.ctx_compress(combined_feature)
...
@registry.ROI_RELATION_PREDICTOR.register("IMPKGCPredictor")
class IMPKGCPredictor(nn.Module):
    def __init__(self, config, in_channels):
        super(IMPKGCPredictor, self).__init__()
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        self.use_bias = False
        ...
        self.context_layer = IMPContext(config, self.num_obj_cls, self.num_rel_cls, in_channels)
        self.kg_layer = KGCContext(config)
        self.rel_compress = nn.Linear(2*self.num_rel_cls, self.num_rel_cls, bias=True)
        # post decoding
        self.hidden_dim = config.MODEL.ROI_RELATION_HEAD.CONTEXT_HIDDEN_DIM
        self.pooling_dim = config.MODEL.ROI_RELATION_HEAD.CONTEXT_POOLING_DIM
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        kg_dists = self.kg_layer(pair_pred)
        if kg_dists.dtype != rel_dists.dtype:
            kg_dists = kg_dists.to(rel_dists.dtype)
        combined_feature = torch.cat((rel_dists, kg_dists), dim=1)
        rel_dists = self.rel_compress(combined_feature)
...
@registry.ROI_RELATION_PREDICTOR.register("IMPKGC3Predictor")
class IMPKGC3Predictor(nn.Module):
    def __init__(self, config, in_channels):
        super(IMPKGC3Predictor, self).__init__()
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        self.use_bias = False
        ...
        self.context_layer = IMPContext(config, self.num_obj_cls, self.num_rel_cls, in_channels)
        self.kg_layer = TransEmbedding(config)
        self.rel_compress = nn.Linear(2*self.num_rel_cls, self.num_rel_cls, bias=True)
        # post decoding
        self.hidden_dim = config.MODEL.ROI_RELATION_HEAD.CONTEXT_HIDDEN_DIM
        self.pooling_dim = config.MODEL.ROI_RELATION_HEAD.CONTEXT_POOLING_DIM
...
class MotifPredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        # print(pair_pred.min(),pair_pred.max())
...
@registry.ROI_RELATION_PREDICTOR.register("MotifKGCPredictor")
class MotifKGCPredictor(nn.Module):
    def __init__(self, config, in_channels):
        super(MotifKGCPredictor, self).__init__()
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        ...
        self.post_cat = nn.Linear(self.hidden_dim * 2 + self.num_rel_cls, self.pooling_dim)
        self.rel_compress = nn.Linear(self.pooling_dim, self.num_rel_cls, bias=True)
        # self.fuse_layer = nn.Linear(self.num_rel_cls * 2, self.num_rel_cls, bias=True)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        if kg_dists.dtype != prod_rep.dtype:
            kg_dists = kg_dists.to(prod_rep.dtype)
        combined_feature = torch.cat((prod_rep, kg_dists), dim=1)
        prod_rep = self.post_cat(combined_feature)
        ...
        # print(self.freq_bias.index_with_labels(pair_pred.long()))
...
@registry.ROI_RELATION_PREDICTOR.register("MotifKGC3Predictor")
class MotifKGC3Predictor(nn.Module):
    def __init__(self, config, in_channels):
        super(MotifKGC3Predictor, self).__init__()
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        ...
        self.trans_embedding = TransEmbedding(config)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        trans_embs = self.trans_embedding(pair_pred)
        if trans_embs.dtype != prod_rep.dtype:
            kg_dists = trans_embs.to(prod_rep.dtype)
        combined_feature = torch.cat((prod_rep, trans_embs), dim=1)
        prod_rep = self.post_cat(combined_feature)
...
@registry.ROI_RELATION_PREDICTOR.register("MotifKGC2Predictor")
class MotifKGC2Predictor(nn.Module):
    def __init__(self, config, in_channels):
        super(MotifKGC2Predictor, self).__init__()
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        rel_dists = torch.cat([rel_dists, kg_dists], dim=1)
        rel_dists = self.fuse_layer(rel_dists)
...
@registry.ROI_RELATION_PREDICTOR.register("MotifKGC4Predictor")
class MotifKGC4Predictor(nn.Module):
    def __init__(self, config, in_channels):
        super(MotifKGC4Predictor, self).__init__()
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        rel_dists = self.rel_compress(prod_rep)
        trans_embeds = self.trans_embedding(pair_pred)

        if trans_embeds.dtype != rel_dists.dtype:
            kg_dists = trans_embeds.to(rel_dists.dtype)

        rel_dists = torch.cat([rel_dists, trans_embeds], dim=1)
        rel_dists = self.fuse_layer(rel_dists)
...
@registry.ROI_RELATION_PREDICTOR.register("VCTreeKGCPredictor")
class VCTreeKGCPredictor(nn.Module):
    def __init__(self, config, in_channels):
        super(VCTreeKGCPredictor, self).__init__()
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
...
@registry.ROI_RELATION_PREDICTOR.register("VCTreeKGC3Predictor")
class VCTreeKGC3Predictor(nn.Module):
    def __init__(self, config, in_channels):
        super(VCTreeKGC3Predictor, self).__init__()
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
...
@registry.ROI_RELATION_PREDICTOR.register("VCTreeKGC2Predictor")
class VCTreeKGC2Predictor(nn.Module):
    def __init__(self, config, in_channels):
        super(VCTreeKGC2Predictor, self).__init__()
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        ...
        # self.frq_gate = nn.Linear(self.pooling_dim, self.num_rel_cls)
        self.ctx_compress = nn.Linear(self.pooling_dim, self.num_rel_cls)
        self.rel_compress = nn.Linear(2*self.num_rel_cls, self.num_rel_cls)
        # self.uni_compress = nn.Linear(self.pooling_dim, self.num_rel_cls)
        # layer_init(self.uni_gate, xavier=True)
        # layer_init(self.frq_gate, xavier=True)
        layer_init(self.ctx_compress, xavier=True)
...
@registry.ROI_RELATION_PREDICTOR.register("VCTreeKGC4Predictor")
class VCTreKGC4Predictor(nn.Module):
    def __init__(self, config, in_channels):
        super(VCTreKGC4Predictor, self).__init__()
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
...
@registry.ROI_RELATION_PREDICTOR.register("TDEKGCPredictor")
class TDEKGCPredictor(nn.Module):
    def __init__(self, config, in_channels):
        super(TDEKGCPredictor, self).__init__()
        self.cfg = config
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.spatial_for_vision = config.MODEL.ROI_RELATION_HEAD.CAUSAL.SPATIAL_FOR_VISION
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        rel_dists = self.calculate_logits(union_features, post_ctx_rep, pair_pred, use_label_dist=False, use_bias=self.use_bias)
        kg_dists = self.kg_layer(pair_pred)
        if kg_dists.dtype != rel_dists.dtype:
            kg_dists = kg_dists.to(rel_dists.dtype)
        combined_feature = torch.cat((rel_dists, kg_dists), dim=1)
        ...
        if self.training:
            ...
            if self.effect_type == 'TDE':  # TDE of CTX
                rel_dists = self.calculate_logits(union_features, post_ctx_rep, pair_obj_probs) - self.calculate_logits(
                    union_features, avg_ctx_rep, pair_obj_probs, use_bias=self.use_bias)
            elif self.effect_type == 'NIE':  # NIE of FRQ
                rel_dists = self.calculate_logits(union_features, avg_ctx_rep, pair_obj_probs) - self.calculate_logits(
                    union_features, avg_ctx_rep, avg_frq_rep, use_bias=self.use_bias)
            elif self.effect_type == 'TE':  # Total Effect
                rel_dists = self.calculate_logits(union_features, post_ctx_rep, pair_obj_probs) - self.calculate_logits(
                    union_features, avg_ctx_rep, avg_frq_rep, use_bias=self.use_bias)
            else:
    ...
    def calculate_logits(self, vis_rep, ctx_rep, frq_rep, use_label_dist=True, mean_ctx=False, use_bias = True):
        if use_label_dist:
            frq_dists = self.freq_bias.index_with_probability(frq_rep)
        else:
            frq_dists = self.freq_bias.index_with_labels(frq_rep.long())
        ...
        if self.fusion_type == 'gate':
            ...
            if use_bias:
                ...
                union_dists = ctx_dists * torch.sigmoid(vis_dists + ctx_gate_dists)
            # union_dists = (ctx_dists.exp() * torch.sigmoid(vis_dists + frq_dists + ctx_constraint) + 1e-9).log()    # improve on zero-shot, but low mean recall and TDE recall
            # union_dists = ctx_dists * torch.sigmoid(vis_dists * frq_dists)                                          # best conventional Recall results
            # union_dists = (ctx_dists.exp() + vis_dists.exp() + frq_dists.exp() + 1e-9).log()                        # good zero-shot Recall
            # union_dists = ctx_dists * torch.max(torch.sigmoid(vis_dists), torch.sigmoid(frq_dists))                 # good zero-shot Recall
